...
def linker(url, freq, vid_len):
    ...
    while Round:
        ...
        if frameId % (frameRate * freq) == 0:
            ...
            out = make_output_vid(frame=frame, frameId=frameId, out=out)
            x = temp
            temp = frameId
            f = f + 1
...
while True:
    ori, den_map, cnt, f_mp, flow_dir, flow_mag, fight_l, abnormal_l, pose = linker(
        'rtsp://root:pass@10.144.129.107/axis-media/media.amp', 5, 5)
