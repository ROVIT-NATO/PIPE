...
import algos.counting.C_CNN.src.network as network
from algos.counting.C_CNN.src.models import CMTL
...
class CrowdCounter(nn.Module):
    ...
    def forward(self,  im_data, gt_data=None, gt_cls_label=None, ce_weights=None):        
        ...
        if self.training:                        
            ...
            gt_cls_label = \
                network.np_to_variable(gt_cls_label, is_cuda=True, is_training=self.training,dtype=torch.FloatTensor)
            self.loss_mse, self.cross_entropy = \
                self.build_loss(density_map, density_cls_prob, gt_data, gt_cls_label, ce_weights)
